...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const diasRef = collection(db, "faturamento", unidade, anoAtual, mesAtual, "dias");
          ...
          snapshot.forEach((doc) => {
            ...
            if (data.vendas && Array.isArray(data.vendas)) {
              data.vendas.forEach(venda => {
                ...
                responsaveisSet.add(venda.responsavel);
  ...
  const handleUpload = async () => {
    if (!file || file.size === 0) {
      setError("Selecione um arquivo válido primeiro");
      return;
    }
    ...
    if (!allowedTypes.includes(file.type)) {
      setError("Tipo de arquivo não suportado. Apenas arquivos Excel (.xls, .xlsx) são permitidos.");
      return;
    }
          ...
          <div className="filter-group">
            ...
            <div className="filter-stats">
              <span>
                ...
                Total: {totalFiltrado.toLocaleString('pt-BR', {
                  style: 'currency',
                  currency: 'BRL'
                })} | 
                Média: {mediaPorVenda.toLocaleString('pt-BR', {
                  style: 'currency',
                  currency: 'BRL'
                })}
              </span>
              ...
              <table>
                ...
                <tbody>
                  {vendasFiltradas.length > 0 ? (
                    vendasFiltradas
                      .sort((a, b) => dayjs(b.dataFormatada) - dayjs(a.dataFormatada))
                      .map((venda, index) => (
                        <tr key={index}>
                          <td>{dayjs(venda.dataFormatada).format('DD/MM/YYYY')}</td>
                          <td>{venda.produto}</td>
                          <td>{venda.nome}</td>
                    ...
                    <tr>
                      <td colSpan="6">Nenhuma venda encontrada com os filtros atuais</td>
                    </tr>
                  )}
                </tbody>
              </table>
